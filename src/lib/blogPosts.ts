...
export const blogPosts: BlogPost[] = [
  {
    ...
    intro:
      "Most Core Web Vitals advice stops at plugin recommendations and generic checklists. In practice, the fixes that actually move LCP, INP, and CLS on a production site are architectural. It comes down to how your data is fetched, how many times it's fetched, and what runs before the browser paints anything. This is what I've found actually works, based on real production sites I've audited and fixed.",
    sections: [
      {
...
export async function getProducts() {
  ...
  return res.json();
      ...
      },
      {
        ...
        paragraphs: [
          "On the Nexus Clinic rebuild, where I migrated an aesthetic clinic's site from traditional WordPress to headless WordPress with a Next.js front end, the combination of server-rendered pages, a secured API layer, and a full technical optimization pass landed 0.8s Largest Contentful Paint and 30ms Total Blocking Time on desktop, with 95+ Performance on mobile and 100 on desktop. None of that came from a single silver-bullet setting. It came from treating rendering strategy, data fetching, and JavaScript weight as one system instead of three separate audits.",
        ],
      },
    ],
    takeaways: [
      'LCP is usually a data-fetching and caching problem before it becomes an image problem.',
      'INP tracks the full interaction, so audit main-thread JavaScript, not just input delay.',
...
export default nextConfig;`,
    ...
    takeaways: [
      ...
      'Verify structured data parity before launch, and track indexed pages and rankings, not just Lighthouse scores.',
    ],
    relatedCaseStudy: { slug: 'nexus-clinic-headless-wordpress-nextjs', label: 'Read the full Nexus Clinic case study' },
  },
  {
...
add_action('wp_enqueue_scripts', function () {
    ...
    takeaways: [
      ...
      'Load test checkout specifically, since its failure pattern is different from a homepage or product page.',
    ],
    relatedCaseStudy: {
  ...
  },
  {
    ...
    sections: [
      ...
      {
        heading: 'Authenticated, scoped rooms instead of a public broadcast',
        paragraphs: [
...
function broadcastBid(auctionId, bid) {
  io.to('auction:' + auctionId).emit('bid:update', bid);
    ...
    takeaways: [
      ...
      'Scope every real-time event to an authenticated room, and never broadcast live data site-wide.',
  ...
  },
  {
    ...
    sections: [
      ...
      },
      {
        ...
        paragraphs: [
          "Server-side rendering earns its cost (a render on every request) when the page genuinely can't be the same for two visitors: authenticated dashboards, personalized content, or pages whose data changes faster than any reasonable cache window. On the Nexus Clinic rebuild, the front end is server-rendered specifically for the SEO control that comes with it, meaning full control over meta tags and structured data per request, generated from live headless WordPress content, while the secured API boundary keeps that per-request render fast rather than a source of latency.",
        ],
      },
      {
...
export async function generateStaticParams() {
  ...
  return posts.map((post) => ({ slug: post.slug }));
      ...
      },
      {
        ...
        paragraphs: [
          "On the flooring retailer rebuild, the site was already using a reasonable rendering strategy on Vercel. The actual problem was that every page was independently re-fetching the same CMS API data, with no caching layer coordinating any of it. Introducing a centralized cache for products, blogs, and site data mattered more than the SSG, SSR, and ISR label on any individual route, because uncached data fetching inside any rendering strategy will still be slow. Rendering strategy and data-fetching architecture are two separate decisions, and you need to get both right, not just one.",
        ],
      },
    ],
    takeaways: [
